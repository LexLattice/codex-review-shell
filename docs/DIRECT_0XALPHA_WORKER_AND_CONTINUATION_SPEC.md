...
OpenCode is launched in a neutral runtime directory with project configuration,
default plugins, and LSP downloads disabled. Its inline constitution denies all
...
The complete text stream is persisted through the normal native-child
epistemic capture. It is not flattened into the parent transcript. This keeps
the higher-level agent's context bounded while preserving an addressable source
for later semantic import or audit.
...
missing-session recovery, OpenCode `unknown` continuation, tool-boundary
failure, provider selection, model binding, workspace exclusion, trace-digest
admission, and secret-safe projections.
