...
- POSIX commands use process groups. Windows-local cancellation uses verified `taskkill /PID … /T /F`; any command failure or missing child-exit observation remains unquiesced.
- Shutdown helpers enforce: stop intake, verify restart reconciliation, retry settlement, request cancellation, await child/provider acknowledgement, drain backend requests, dispose backends, close registry.
