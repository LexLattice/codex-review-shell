...
| PR | Status | Branch | Purpose | Planned deliverable | Explicit non-goals |
| --- | --- | --- | --- | --- | --- |
| `#109` | merged | `codex/direct-external-capability-profile` | External capability profile | ExternalCapabilityProfile, McpServerIdentityWitness, McpServerSelector blockers, descriptorKind/executionState rows for discovery/read/action/plugin families, blocked/deferred dynamic/plugin rows, resident/operator compact witness | No provider declarations, resource reads, dynamic MCP execution, or plugin install |
...
| `WM-K6` plan-contract execution continuation | implemented and locally regression-proven | Durable revisioned execution state; implementation `ResolvedTaskConstitution`; trusted role handoff; exact single-use operator worker-start authority; real Direct worker/session/turn; per-call local tool gates; local-only capability/authority/prompt/evaluator agreement; fixed Project Manager semantic closure assessment bound to runtime evidence; fail-closed criterion witness validation; completed WorkThread; project-memory candidate; project-scoped canonical admission and upward status; restart and Playwright proof | Worker activity is never completion, the worker cannot write project/world state, and missing or foreign criterion evidence remands closure. General tool-request presentation and multi-project execution controls remain renderer-hardening work. |
