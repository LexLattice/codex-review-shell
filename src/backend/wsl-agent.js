...
function createStoredPresentationBuilder(threadId, sourceFile) {
  const turns = [];
...
async function readCodexThreadTranscript(params = {}) {
  ...
  const entries = [];
  const presentationBuilder = createStoredPresentationBuilder(threadId, session.sessionFilePath);
  let lineCount = 0;
