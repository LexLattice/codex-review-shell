...
class UsageLedgerStore {
  ...
  status() {
    return {
      ...
      ledgerLabel: path.basename(this.ledgerPath),
      manifestLabel: path.basename(this.manifestPath),
      ledgerPathEvidenceKey: `usage-ledger-path:${sha256(this.ledgerPath).slice(0, 16)}`,
      manifestPathEvidenceKey: `usage-ledger-manifest:${sha256(this.manifestPath).slice(0, 16)}`,
