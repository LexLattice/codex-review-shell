...
function summarizeAgentRegistry(input = {}) {
  ...
  return {
    ...
    activeCount: Number(status.activeCount || projection.activeCount || 0),
    runCount: Number(status.runCount || projection.runCount || 0),
    activeRunCount: Number(status.activeRunCount || projection.activeRunCount || 0),
    threadLinkCount: Number(status.threadLinkCount || 0),
    projectionDigest: normalizeString(status.projectionDigest || projection.projectionDigest, ""),
    backfillTouchedAgentCount: Number(backfillReport.touchedAgentCount || 0),
    backfillTouchedAgentRunCount: Number(backfillReport.touchedAgentRunCount || 0),
...
function summarizeProviderHostedTools(hostedTools = {}) {
  ...
  return {
    ...
    activationSnapshotDigest: normalizeString(status.activationSnapshotDigest, ""),
    activationReadyCount: Number(status.activationReadyCount || 0),
    declarationDecisionCount: Number(status.declarationDecisionCount || 0),
    blockedDecisionCount: Number(status.blockedDecisionCount || 0),
    ...
    webSearchActivationDecision: normalizeString(arrayOrEmpty(status.activationSnapshot?.declarationDecisions).find((decision) => decision?.toolKind === "web_search" && decision?.invocationMode === "model_mediated_provider_tool")?.decision, "unknown"),
    imageGenerationResidentDecision: normalizeString(arrayOrEmpty(status.activationSnapshot?.declarationDecisions).find((decision) => decision?.toolKind === "image_generation" && decision?.invocationMode === "model_mediated_provider_tool")?.decision, "unknown"),
    imageGenerationOperatorDecision: normalizeString(arrayOrEmpty(status.activationSnapshot?.declarationDecisions).find((decision) => decision?.toolKind === "image_generation" && decision?.invocationMode === "operator_triggered_provider_operation")?.decision, "unknown"),
...
function buildRows(sections) {
  ...
  return {
    ...
    agents: [
      ...
      statusRow("Runs / active", `${agents.runCount}/${agents.activeRunCount}`),
      statusRow("Thread links", agents.threadLinkCount),
      statusRow("Backfill touched", `${agents.backfillTouchedAgentCount} agents / ${agents.backfillTouchedAgentRunCount} runs / ${agents.backfillTouchedThreadLinkCount} links`, agents.backfillTouchedAgentCount || agents.backfillTouchedAgentRunCount || agents.backfillTouchedThreadLinkCount ? "diagnostic" : "ok"),
