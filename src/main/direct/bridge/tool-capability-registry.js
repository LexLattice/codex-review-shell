...
function defaultToolCapabilityInputs() {
  return [
    ...
    },
    {
      ...
      sideEffectClass: "process_session",
      authorityRequired: "process_session_gate",
      requestShapeFamilies: ["stateful_exec_session_envelope", "bounded_output_frame", "quoted_tool_result_continuation"],
      localExecutor: "src/main/direct/tools/stateful-exec-session.js",
      approvalMode: "per_session",
      providerResultEnvelopeType: "bounded_result_envelope",
      recoveryLaw: "Stateful process sessions require session identity, bounded output frames, cancellation, cleanup, stdin policy, and restart recovery classification; running sessions are never terminal success.",
      failureClasses: ["projection_laundering", "authority_inflation", "context_smuggling", "action_replay"],
      rendererSafeSummary: "Implemented as restricted stateful process-session evidence with plain-pipe mode, bounded output frames, idle/hard timeout posture, cancellation/cleanup plan, and replay-forbidden recovery law; not an alias for run_command.",
    },
    {
      toolId: "vanilla.write_stdin",
      displayName: "write_stdin",
      vanillaNames: ["write_stdin"],
      directNames: ["write_stdin"],
      odeuFamily: "workspace_process_authority",
      capabilityState: "runtime_probed",
      implementationState: "restricted_executor",
      promotionState: "direct_restricted",
      ...
      sideEffectClass: "process_session",
      authorityRequired: "process_session_gate",
      requestShapeFamilies: ["stateful_stdin_envelope", "stateful_exec_session_envelope"],
      localExecutor: "src/main/direct/tools/stateful-exec-session.js",
      approvalMode: "per_action",
      providerResultEnvelopeType: "bounded_result_envelope",
      recoveryLaw: "Stdin write is authority-bearing and can target only known live sessions whose command class declares a stdin policy; terminal or missing sessions are blocked.",
      failureClasses: ["projection_laundering", "authority_inflation", "context_smuggling", "action_replay"],
      rendererSafeSummary: "Implemented as a restricted stdin event plan over a known stateful exec session, with metadata-only input projection and missing/terminal target blockers.",
    },
    {
    ...
    },
    {
      ...
      providerResultEnvelopeType: "permission_widening_decision",
      ...
      recoveryLaw: "Permission widening produces only a single-action request/status packet in Wave 17; a separate operator decision is required before any future grant.",
    ...
    },
    {
      ...
      authorityRequired: "workspace_read_gate",
      requestShapeFamilies: ["image_view_staging_envelope@1"],
      approvalMode: "display_only",
      contextVisibility: "result_envelope_only",
      providerResultEnvelopeType: "image_metadata_projection",
      rawExposurePolicy: "metadata_only",
      recoveryLaw: "Renderer preview, file metadata, provider image payload, and model pixel perception are separate evidence states; Wave 17 never sends image payloads and always reports modelSawPixels=false.",
    ...
    {
      toolId: "direct.inspect_self_constitution",
      displayName: "inspect_self_constitution",
      directNames: ["inspect_self_constitution"],
      odeuFamily: "session_control_state",
      capabilityState: "runtime_probed",
      implementationState: "full_executor",
      promotionState: "direct_enabled",
      providerDeclarationState: "declared_live_accepted",
      localExecutorState: "implemented_full",
      localExecutor: "src/main/direct/bridge/self-constitution.js",
      sideEffectClass: "session_state",
      authorityRequired: "display_only",
      requestShapeFamilies: ["direct_self_constitution_snapshot@1"],
      approvalMode: "display_only",
      contextVisibility: "result_envelope_only",
      providerResultEnvelopeType: "bounded_result_envelope",
      rawExposurePolicy: "metadata_only",
      recoveryLaw: "Current self-knowledge is compiled from owner-issued harness state; prompt prose is presentation and cannot override the snapshot.",
      failureClasses: ["projection_laundering", "authority_inflation", "context_smuggling", "renderer_authority_leak"],
    },
    {
    ...
    })),
    {
      ...
      providerDeclarationState: "not_declared",
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/agents/batch-job-surface.js",
      requestShapeFamilies: ["direct_batch_agent_job_plan", "direct_batch_agent_worker_item", "direct_batch_agent_result_contract", "direct_batch_agent_aggregation_ledger"],
      approvalMode: "future_gate_required",
      unsupportedReason: "batch_execution_blocked_until_job_authority",
      usageAttribution: "batch_job",
      agentEligibility: "primary_only",
      recoveryLaw: "Batch spawn is fan-out intent only until job identity, worker items, result contract, aggregation ledger, and separate usage attribution are proved.",
    ...
    },
    {
      ...
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/agents/batch-job-surface.js",
      requestShapeFamilies: ["direct_batch_agent_result_contract", "direct_batch_agent_aggregation_ledger", "bounded_result_envelope"],
      approvalMode: "future_gate_required",
      unsupportedReason: "worker_result_report_blocked_until_batch_ledger_authority",
      usageAttribution: "batch_job",
      agentEligibility: "batch_worker_only",
      providerResultEnvelopeType: "agent_mailbox_event",
      recoveryLaw: "Worker result reporting requires worker-item identity, result evidence refs, duplicate detection, and aggregation ledger reconciliation before any parent success claim.",
      failureClasses: ["projection_laundering", "authority_inflation", "context_smuggling", "thread_flattening"],
    },
    {
      ...
      providerDeclarationState: "not_declared",
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/external/capability-discovery.js",
      requestShapeFamilies: ["external_capability_descriptor"],
      approvalMode: "display_only",
      contextVisibility: "diagnostic_only",
    ...
    ...["list_mcp_resources", "list_mcp_resource_templates"].map((name) => ({
      ...
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/external/capability-discovery.js",
      requestShapeFamilies: ["external_capability_descriptor", "external_resource_descriptor"],
      approvalMode: "display_only",
      contextVisibility: "diagnostic_only",
      failureClasses: ["projection_laundering", "authority_inflation", "renderer_authority_leak"],
    })),
    {
      ...
      providerDeclarationState: "not_declared",
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/external/mcp-boundary.js",
      sideEffectClass: "external_read",
      requestShapeFamilies: ["mcp_resource_read_boundary", "external_source_provenance", "external_evidence_ref"],
      approvalMode: "future_gate_required",
    ...
    },
    {
      ...
      requestShapeFamilies: ["mcp_dynamic_tool_call_boundary", "external_source_provenance", "bounded_result_envelope"],
      approvalMode: "future_gate_required",
    ...
    },
    {
      ...
      providerDeclarationState: "not_declared",
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/external/plugin-governance.js",
      requestShapeFamilies: ["external_capability_descriptor", "plugin_catalog_descriptor", "plugin_governance_status"],
      approvalMode: "display_only",
    },
    {
      ...
      requestShapeFamilies: ["plugin_install_request_posture", "plugin_capability_diff", "plugin_rollback_law"],
      approvalMode: "future_gate_required",
    ...
    ...["code_mode_execute", "code_mode_wait"].map((name) => ({
      ...
      providerDeclarationState: "not_declared",
      localExecutorState: "scaffolded",
      localExecutor: "src/main/direct/tools/code-mode-execution-lane.js",
      requestShapeFamilies: name === "code_mode_execute"
        ? ["code_mode_execution_lane_status", "code_mode_kernel_session", "code_mode_execute_request_posture", "code_mode_artifact_output_policy"]
        : ["code_mode_execution_lane_status", "code_mode_kernel_session", "code_mode_wait_cancel_policy"],
      approvalMode: "future_gate_required",
      unsupportedReason: "execution_blocked_until_code_mode_kernel_authority",
